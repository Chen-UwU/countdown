...
class Style:
    ...
    def to_dict(self) -> dict:
        ...
        return dict(zip(needed, [self[i] for i in needed]))
...
class Event:
    ...
    def __init__(
        self, event: str, start: datetime, end: datetime, style: StyleGroup
    ) -> None:
        self.event = event
        self.start = start
        self.end = end
...
def add_event(event: str, start: datetime, end: datetime, style: StyleGroup) -> None:
    """添加新事件"""
    new_event = Event(event, start, end, style)
